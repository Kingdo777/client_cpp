...
#define DEFAULT_MAIN_FUNC "rfit_main"
...
const char *getPythonFunctionName() {
    char *entryFunc = rfitGetPythonEntry();
...
int main(int argc, char *argv[]) {
    ...
    char *user = rfitGetPythonUser();
    char *funcName = rfitGetPythonFunc();
    ...
    if (module != nullptr) {
        ...
        if (func && PyCallable_Check(func)) {
            ...
            if (strcmp(pythonFuncName, DEFAULT_MAIN_FUNC) == 0) {
                ...
                long inputSize = rfitGetInputSize();
                if (inputSize == 0) {
                    // No input from Faasm
                    ...
                    rfitGetInput(rawInput, inputSize);
