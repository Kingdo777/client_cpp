...
# Install dirs
RFIT_LOCAL_DIR=/home/kingdo/CLionProjects/client_cpp/runtime
PREFIX=$(RFIT_LOCAL_DIR)/toolchain
RFIT_SYSROOT=/home/kingdo/CLionProjects/client_cpp/runtime/llvm-sysroot
...
extras: $(BUILD_DIR)/libcxxabi.BUILT
	cp $(TOOLCHAIN_DIR)/sysroot_extras/* $(RFIT_SYSROOT)/lib/wasm32-wasi/
...
$(BUILD_DIR)/libcxx.BUILT: $(BUILD_DIR)/compiler-rt.BUILT
	...
	cd $(BUILD_DIR)/libcxx; cmake -G Ninja \
		...
		-DCMAKE_VERBOSE_MAKEFILE:BOOL=ON \
		-DCMAKE_INSTALL_PREFIX=$(RFIT_SYSROOT) \
...
$(BUILD_DIR)/libcxxabi.BUILT: $(BUILD_DIR)/libcxx.BUILT
	...
	cd $(BUILD_DIR)/libcxxabi; cmake -G Ninja \
		-DCMAKE_BUILD_TYPE=RelWithDebugInfo \
		-DCMAKE_C_FLAGS="-I$(RFIT_SYSROOT)/include" \
		-DCMAKE_CXX_FLAGS="-I$(RFIT_SYSROOT)/include/c++/v1" \
		-DCMAKE_TOOLCHAIN_FILE=$(TOOLCHAIN_FILE) \
	        -DLLVM_CONFIG_PATH=$(LLVM_CONFIG) \
		-DCMAKE_CXX_COMPILER_WORKS=ON \
		-DCMAKE_C_COMPILER_WORKS=ON \
		...
		-DLIBCXXABI_LIBCXX_INCLUDES=$(RFIT_SYSROOT)/include/c++/v1 \
		-DUNIX:BOOL=ON \
